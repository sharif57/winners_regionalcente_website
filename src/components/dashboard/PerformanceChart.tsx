...
export default function PerformanceChart() {
  ...
  return (
    <div className="w-full overflow-hidden border border-gray-100 bg-white p-4 text-[#1F1F1F] sm:p-6 lg:p-8">
      {/* Header */}
      <div className="mb-6 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h2 className="text-xl font-semibold italic text-secondary sm:text-2xl">
        ...
        <div className="flex w-full rounded-xl bg-gray-100 p-1 sm:w-auto">
          {["Yearly", "Monthly"].map((mode) => (
            <button
              ...
              onClick={() => setView(mode as "Yearly" | "Monthly")}
              className={cn(
                "flex-1 rounded-lg px-4 py-2 text-[11px] font-bold uppercase tracking-wider transition-all sm:flex-none sm:px-6 sm:text-xs",
                view === mode
                  ? "bg-white text-[#1F1F1F] shadow-sm"
                  : "text-gray-500 hover:text-[#1F1F1F]"
              )}
      ...
      <div className="h-[240px] w-full sm:h-[280px] lg:h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <defs>
              <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#0650B1" stopOpacity={0.7} />
                <stop offset="95%" stopColor="#F5F8FF" stopOpacity={0.95} />
              </linearGradient>
            </defs>
